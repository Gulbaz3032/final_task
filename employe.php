...
$sql = "SELECT * FROM users_data LIMIT $offset, $rows_per_page";
$res = $connection->query($sql);

if($_POST) {
  $search_term = $_POST['search_term'];

  $sql2 = "SELECT * FROM users_data WHERE name LIKE '%{$search_term}%'OR email LIKE '{$search_term}'";
...
<table class="table table-borderless table-hover table-stripes">
  ...
  if($res->num_rows > 0) {
    ...
    <td> <?php echo $row['frontend_skills']; ?></td>
    <td> <?php echo $row['backend_skills']; ?></td>
